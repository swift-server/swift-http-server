...
> Inline (in-memory) credentials, reloading credentials, and mTLS are currently not supported over HTTP/3. Note that the
> `HTTP3` trait must be enabled for HTTP/3 configuration to be parsed.
...
| `http.http3.datagramConfiguration`  | `maxDatagramFrameSize`                    | `int` (bytes)   | Optional                                                                                                                      | 65535          |
| `http.http3.datagramConfiguration`  | `maxBufferedDatagrams`                    | `int`           | Optional                                                                                                                      | 16             |
